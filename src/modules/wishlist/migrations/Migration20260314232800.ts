...
export class Migration20260314232800 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "wishlist" ("id" text not null, "customer_id" text not null, "sales_channel_id" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "wishlist_pkey" primary key ("id"));`);
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_wishlist_customer_sales_channel_unique" ON "wishlist" ("customer_id", "sales_channel_id") WHERE deleted_at IS NULL;`);
    ...
    this.addSql(`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_wishlist_item_variant_wishlist_unique" ON "wishlist_item" ("product_variant_id", "wishlist_id") WHERE deleted_at IS NULL;`);
  ...
  override async down(): Promise<void> {
    this.addSql(`alter table "wishlist_item" drop constraint if exists "wishlist_item_wishlist_id_foreign";`);
