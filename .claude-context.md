# Project Context - VitaIntegral Store

## Project Overview
- **Project**: VitaIntegral Store - Natural & Organic Food E-commerce
- **Backend**: Medusa 2.11.3
- **Frontend**: Next.js (medusapando-front)
- **Node Version**: >= 20
- **Package Manager**: Yarn (with .yarnrc.yml)

## Brand Identity
- **Focus**: Conscious, natural, organic foods
- **Values**: Coherencia, Consciencia, Servimos con Amor, Creamos Valor
- **Target**: Health-conscious customers in LATAM (Spanish)
- **Primary Color**: Emerald Green (#5B8C3E)

## Frontend Structure

### Key Components

#### Hero Section (`src/modules/home/components/hero/hero-client.tsx`)
- Interactive banner with zoom effect on hover
- Uses local image `/public/banner1.webp`
- White gradient overlay for text readability
- Spanish LATAM content
- "Ver Categorías" button opens side menu
- Trust indicators: 100% Natural, Entrega 24-72h, Compra Segura

#### Navigation Header (`src/modules/layout/components/nav-header/index.tsx`)
- Two-row sticky header
- Top row: Logo, Search, Account, Cart (collapses on scroll)
- Bottom row: Categories button, Nav links, compact icons when scrolled
- Mobile responsive: shorter button text, hidden logo when scrolled

#### Side Menu (`src/modules/layout/components/side-menu/index.tsx`)
- Controlled via `SideMenuContext` (allows opening from anywhere)
- Shows "Categorías" on mobile, "Comprar por Categorías" on desktop
- Slide-in panel with categories

#### Side Menu Context (`src/lib/context/side-menu-context.tsx`)
- Shared state for side menu open/close
- Used by both nav header and hero section
- Provider wraps main layout
...
- [x] Hero section with interactive banner
- [x] Side menu context for shared state
- [x] Mobile-responsive header and components
