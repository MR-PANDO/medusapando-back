...
export const SeoMetadata = model
  .define("seo_metadata", {
    ...
    seo_keywords: model.json().default([]),
    ...
    structured_data_json: model.json().default({}),
    sitemap_priority: model.float().default(0.5),
    sitemap_changefreq: model.text().default("weekly"),
    hreflang_entries: model.json().default([]),

    // AEO Fields
    aeo_faqs: model.json().default([]),
    aeo_howto_steps: model.json().default([]),
    aeo_short_answer: model.text().nullable(),

    // GEO Fields
    geo_entity_summary: model.text().nullable(),
    geo_citations: model.json().default([]),
    geo_key_attributes: model.json().default([]),

    // SXO Fields
    sxo_intent: model.text().nullable(),
    sxo_cta_text: model.text().nullable(),
    sxo_internal_links: model.json().default([]),
