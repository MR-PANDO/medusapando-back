...
const NubexPage = () => {
  ...
  const fetchLogs = async () => {
    setLoading(true)
    try {
      const res = await fetch("/admin/nubex?limit=20", {
        credentials: "include",
      })
      if (res.ok) {
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b hover:bg-ui-bg-subtle-hover">
                    ...
                    <td className="px-4 py-3">
                      {log.errors > 0 ? (
                        <span className="text-ui-fg-error" title={log.error_details ?? ""}>
                          {log.errors}
                        </span>
                      ) : (
                        "0"
