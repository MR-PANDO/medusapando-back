...
export async function runNubexSync(
  ...
  try {
    ...
    // 2. Query all Medusa variants with their SKUs
    ...
    // 5. Query inventory items linked to matched variants
    ...
    if (matchedVariants.length > 0) {
      ...
      console.log(
        `[NubexSync] Inventory creates: ${inventoryCreates.length}, updates: ${inventoryUpdates.length}, skipped (no location): ${result.skipped_no_inventory}`
      )
    }
    ...
    // 6. Create missing inventory levels in batches
    if (inventoryCreates.length > 0) {
      const inventoryService = container.resolve(Modules.INVENTORY) as any

      for (let i = 0; i < inventoryCreates.length; i += BATCH_SIZE) {
        const batch = inventoryCreates.slice(i, i + BATCH_SIZE)
        try {
