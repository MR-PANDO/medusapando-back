...
export async function runNubexSync(
  ...
  try {
    ...
    // 9. Publish/unpublish — DISABLED
    // Products are always visible on the storefront (out-of-stock shown with badge).
    // No publish/unpublish based on stock.
    ...
    // 11. Low-stock notifications moved to separate job (nubex-low-stock-notification.ts)
    // Runs twice a day instead of every sync to avoid email spam.
